...
class UnvalidatedReference(entities.BaseEntity):
    referenced_model_key = review.KeyProperty()


class ValidatedReference(entities.BaseEntity):
    referenced_model_key = review.KeyProperty(kind=ReferencedModel.kind())
...
class ReviewTest(actions.TestBase):
    ...
    def test_constructor_sets_key_name(self):
        ...
        self.assertEqual(
            review.Review.key_name(unit_id, reviewer_key), review_key.name())
